...
namespace Snowplow.Analytics.Extractor.Tests
{
    ...
    public class EventExtractorTest
    {
        ...
        public IRow RowGenerator()
        {
            ...
            var keys = _expected.Properties().Select(p => p.Name).ToList();

            keys.ForEach( key => {
                var type = _expected[key].GetType();
                USqlColumn<string> col = new USqlColumn<string> (key);
                columns.Add(col);
            });

            columns.ForEach(item => Console.WriteLine($"{item.Name}: {item.Type}"));
        ...
        public void TestExtractor()
        {
            ...
            using (MemoryStream stream = GenerateStreamFromString(tsv))
            {
                ...
                var keys = _expected.Properties().Select(p => p.Name).ToList();
                keys.ForEach(key => Console.WriteLine($"{key}: {result[0].Get<string>(key)}"));
            }
